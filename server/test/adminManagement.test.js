...
// name (migration 015) has no visibility gate the way punto_de_venta's
// backfill does — it's just a list column — but the GET path must still
// select it or the client has nothing to render as the row's primary line.
test('GET /api/admins — the list read selects name', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } }, // requireAuth
    { table: 'admins', result: { data: [], error: null } }     // list
  ])
  try {
    ...
    assert.equal(res.status, 200)
    const listSelect = restore.calls.filter((c) => c.table === 'admins' && c.method === 'select').at(-1)
    assert.match(listSelect.args[0], /\bname\b/)
  } finally {
    restore()
  }
})
...
// name is nullable and optional (migration 015) — unlike punto_de_venta,
// omitting it is a legitimate request, not a 400.
test('POST /api/admins — persists name and returns it', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } },                  // requireAuth
    { table: 'admins', result: { data: null, error: { message: 'no rows' } } }, // pre-check
    { table: 'admins', result: { data: { id: 'new-1', email: 'n@x.com', role: 'admin', is_active: true, punto_de_venta: STORE, name: 'Nombre Apellido' }, error: null } } // insert
  ])
  try {
    const res = await request(app)
      .post('/api/admins')
      .set('Authorization', `Bearer ${superToken()}`)
      .send({ email: 'n@x.com', password: 'pw12345', punto_de_venta: STORE, name: 'Nombre Apellido' })
    assert.equal(res.status, 201)
    assert.equal(res.body.name, 'Nombre Apellido')
    const insert = restore.calls.find((c) => c.method === 'insert')
    assert.equal(insert.args[0].name, 'Nombre Apellido')
  } finally {
    restore()
  }
})

test('POST /api/admins — an omitted name is persisted as null', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } },                  // requireAuth
    { table: 'admins', result: { data: null, error: { message: 'no rows' } } }, // pre-check
    { table: 'admins', result: { data: { id: 'new-1', email: 'n@x.com', role: 'admin', is_active: true, punto_de_venta: STORE, name: null }, error: null } } // insert
  ])
  try {
    const res = await request(app)
      .post('/api/admins')
      .set('Authorization', `Bearer ${superToken()}`)
      .send({ email: 'n@x.com', password: 'pw12345', punto_de_venta: STORE })
    assert.equal(res.status, 201)
    assert.equal(res.body.name, null)
    const insert = restore.calls.find((c) => c.method === 'insert')
    assert.equal(insert.args[0].name, null)
  } finally {
    restore()
  }
})

// Mirrors quizzes.title's length guard (see quizzes.js) — the DB CHECK backs
// this up, but the API must reject it before ever issuing an insert.
test('POST /api/admins — a name over 200 chars is rejected (400), and nothing is inserted', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } } // requireAuth
  ])
  try {
    const res = await request(app)
      .post('/api/admins')
      .set('Authorization', `Bearer ${superToken()}`)
      .send({ email: 'n@x.com', password: 'pw12345', punto_de_venta: STORE, name: 'x'.repeat(201) })
    assert.equal(res.status, 400)
    assert.equal(restore.calls.filter((c) => c.method === 'insert').length, 0)
  } finally {
    restore()
...
// name-only (no role/is_active) is a plain column update — same shape as the
// punto_de_venta-only test above, mirrored for the new field.
test('PATCH /api/admins/:id — name-only goes through the plain-update branch', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } }, // requireAuth
    { table: 'admins', result: { data: { id: PLAIN.id, email: PLAIN.email, role: 'admin', is_active: true, punto_de_venta: 'Cerritos', name: 'Nueva Persona' }, error: null } } // update
  ])
  try {
    const res = await request(app)
      .patch(`/api/admins/${PLAIN.id}`)
      .set('Authorization', `Bearer ${superToken()}`)
      .send({ name: 'Nueva Persona' })
    assert.equal(res.status, 200)
    assert.equal(res.body.name, 'Nueva Persona')
    const update = restore.calls.find((c) => c.table === 'admins' && c.method === 'update')
    assert.deepEqual(update.args[0], { name: 'Nueva Persona' })
    // No RPC call: name alone carries none of the last-active-superadmin
    // invariant the RPC exists to serialize.
...
// name (migration 015) extends the same atomic path: a role + punto_de_venta
// + name PATCH must still be exactly one write, carrying every field.
test('PATCH /api/admins/:id — role + punto_de_venta + name travel in a single write', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } }, // requireAuth
    {
      rpc: 'update_admin_role_status',
      result: {
        data: { id: PLAIN.id, email: PLAIN.email, password_hash: 'secret-hash', role: 'superadmin', is_active: true, punto_de_venta: 'Laureles', name: 'Nueva Persona' },
        error: null
      }
    }
  ])
  try {
    const res = await request(app)
      .patch(`/api/admins/${PLAIN.id}`)
      .set('Authorization', `Bearer ${superToken()}`)
      .send({ role: 'superadmin', punto_de_venta: 'Laureles', name: 'Nueva Persona' })
...
// The bug this migration's clear_name flag exists to prevent: COALESCE alone
// can't tell "name not provided" apart from "name explicitly emptied" — both
// arrive as SQL NULL — so without the flag this request would silently keep
// PLAIN's old name while reporting 200 and a role change that DID apply.
test('PATCH /api/admins/:id — role + emptied name in one submit actually clears the name', async () => {
  const restore = mockSupabaseSequence([
    { table: 'admins', result: { data: SUPER, error: null } }, // requireAuth
    {
      rpc: 'update_admin_role_status',
      result: {
        data: { id: PLAIN.id, email: PLAIN.email, password_hash: 'secret-hash', role: 'superadmin', is_active: true, punto_de_venta: STORE, name: null },
        error: null
      }
    }
  ])
