...
export const FORBIDDEN_KEYS = [
  ...
  'correct_answer_text',
  'correctAnswerText',
  // The EXACT key the ADMIN-facing routes/attempts.js uses for the plaintext
  // correct answer (GET /api/attempts/:id → answers[].correctAnswer), which
  // makes it the single most likely accidental copy-paste leak into a
  // usuario-facing route. That admin response is never run through
  // assertNoAnswerLeak — this helper guards /api/user/* only — so listing it
  // here costs the admin side nothing.
  'correct_answer',
  'correctAnswer'
]
