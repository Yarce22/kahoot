...
// name is optional everywhere (no NOT NULL, unlike punto_de_venta) — mirrors
// quizzes.title's length guard, but nothing requires a value at all. Returns
// { error } on a validation failure, or { value } with the normalized value
// (trimmed, empty string collapsed to null) otherwise. Shared by POST and
// PATCH so the two routes can't drift on what counts as a valid name.
function normalizeName(name) {
  if (name === undefined) return { value: undefined }
  if (typeof name !== 'string') return { error: 'name must be a string' }
  const trimmed = name.trim()
  if (trimmed.length > NAME_MAX_LENGTH) return { error: `name must be ${NAME_MAX_LENGTH} characters or fewer` }
...
adminsRouter.get('/', ...superadminOnly, async (req, res, next) => {
  const { data, error } = await supabase
    .from('admins')
    .select('id, email, role, is_active, punto_de_venta, name, created_at')
...
adminsRouter.post('/', ...superadminOnly, async (req, res, next) => {
  const { email, password, role = 'admin', punto_de_venta, name } = req.body ?? {}
  ...
  if (normalizedName.error) return next(httpError(400, normalizedName.error))
  ...
  const { data: admin, error } = await supabase
    .from('admins')
    .insert({ email, password_hash: passwordHash, role, punto_de_venta, name: normalizedName.value ?? null })
    .select('id, email, role, is_active, punto_de_venta, name')
    .single()
...
// PATCH /api/admins/:id — change an admin's role, active status,
// punto_de_venta and/or name (superadmin only). Guards against self-lockout
// and losing the last active superadmin.
//
...
// an operator uses to perform that manual backfill. name (migration 015)
// travels the same way for the same reason as punto_de_venta below: the edit
// popup submits role + punto_de_venta + name together, and it must be one
// write, not several.
adminsRouter.patch('/:id', ...superadminOnly, async (req, res, next) => {
  const { id } = req.params
  const { role, is_active, punto_de_venta, name } = req.body ?? {}

  if (role === undefined && is_active === undefined && punto_de_venta === undefined && name === undefined) {
    return next(httpError(400, 'Provide role, is_active, punto_de_venta and/or name'))
  }
  ...
  if (role !== undefined || is_active !== undefined) {
    ...
    const { data, error } = await supabase.rpc('update_admin_role_status', {
      ...
      // name (migration 015) rides the same transaction for the same reason.
      // clear_name disambiguates "not provided" from "explicitly blanked" —
      // new_name alone can't, since COALESCE treats both as SQL NULL, which
      // would silently keep the old name on a combined role+emptied-name
      // submit (exactly what the edit popup sends).
      new_name: name === undefined ? null : normalizedName.value,
      clear_name: name !== undefined && normalizedName.value === null
    })
  ...
  // punto_de_venta and/or name ALONE is a plain column update: neither carries
  // the last-active-superadmin invariant the RPC exists to serialize. Combined
  // with role/is_active they are NOT reachable here — the RPC above already
  // applied them atomically, and re-writing them separately is what made a
  // combined PATCH partially mutating.
  } else if (punto_de_venta !== undefined || name !== undefined) {
    const changes = {}
    if (punto_de_venta !== undefined) changes.punto_de_venta = punto_de_venta
    if (name !== undefined) changes.name = normalizedName.value
