...
describe('admins store', () => {
  ...
  // name (migration 015) is optional but still forwarded, unlike
  // punto_de_venta it is not required by the server.
  it('createAdmin forwards name in the POST body', async () => {
    post.mockResolvedValue({ id: 'new', email: 'n@x.com', role: 'admin', is_active: true, punto_de_venta: 'Laureles', name: 'Nombre Apellido' })
    const store = useAdminsStore()
    await store.createAdmin({ email: 'n@x.com', password: 'pw', role: 'admin', punto_de_venta: 'Laureles', name: 'Nombre Apellido' })
    expect(post).toHaveBeenCalledWith('/api/admins', {
      email: 'n@x.com',
      password: 'pw',
      role: 'admin',
